---
title: "MRCAUSE_workflow_clumped_alcohol_GIANT_height_Yengo"
author: "KJS"
date: "2024-11-27"
...
# Load exposure data
X1 <- read.table(gzfile("/pl/active/colelab/common/published_gwas/diet/cole_naturecomm_2020/BOLTlmm_UKB_genoQCEURN455146_v3_diet_alcohol_overallfreq.1558.average_QT_BgenSnps_mac20_maf0.005_info0.6.gz"), header = TRUE)
head(X1)
str(X1)

# Load outcome data
X2 <- readRDS("/pl/active/colelab/users/kjames/enviroMR/interim_data/outcome/formatted_GCST006901_Height_Yengo_2018.rds")
head(X2)
str(X2)
```
...
#Some of the outcome data sets (CVD, ALT) didn't have a rsID/SNP column. Thus, we have to merge using variant ID. Create a variant ID column in X1 (diet data) that matches the X2 format (chr:bp:other:effect)
#X1$variant_id <- paste0(X1$CHR, ":", X1$BP, ":", X1$ALLELE0, ":", X1$ALLELE1)
# Also annoying, in CVD outcome dataset the alleles are lowercase
...
X <- cause::gwas_merge(X1, X2, snp_name_cols = c("SNP", "SNP") , #c("variant_id", "variant"), #SNP 
...
Run this for outcomes missing SNP/rsID - CVD, ALT
```{r Deal with missing rsID}
#X1snp <- X1 %>% select(c(SNP, variant_id))
#X_snp <- merge(X, X1snp, by.x = "snp", by.y = "variant_id")

# Rename
#X_snp <- X_snp %>% rename("variant_id" = "snp", "snp" = "SNP")
```

```{r Step 1.5 - Save for LD pruning}
# Step 1.5: Save X for LD pruning on Alpine
# Save X_snp if you ran the chunk above, otherwise save X
saveRDS(X, paste0("/pl/active/colelab/users/kjames/enviroMR/interim_data/MRCAUSE/", exposure, "_", outcome, "_nonLDpruned.rds"))
