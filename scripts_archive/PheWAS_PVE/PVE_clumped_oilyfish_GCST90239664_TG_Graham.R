# MR Analysis: PheWAS-PVE - updated proxies
# Author: KJS
# Date: 11/19/24 
...
exposure <- "oilyfish"
outcome <- "GCST90239664_TG_Graham"
outcome_short <- "TG"
...
# Remove traits with 1329 (oily fish intake)
pwas2 <- pwas2 %>%
  filter(trait != 1329)
...
key_term <- "Oily fish intake" #"Oily fish intake", "Bread consumed" note- alcohol consumed isn't the direct metric but should be a good proxy
...
save(list=c("phewas_thresh_results", "keep_variants", "keep_variant_names", "remove_variants", "remove_variants_names", "filtered_pve"), file = paste0("interim_data/PheWAS_PVE/PheWAS_PVE_interimObjects_",exposure,"_",outcome,".RData"))

#~~~~~~~~~~~~~~~~~~~~~
# 5.1 Add keep column to mrobj based on PVE filter
#~~~~~~~~~~~~~~~~~~~~~
# Update: 11/19/24: We will update mrobj in its own script
...
# Save
#saveRDS(mrobj, paste0("interim_data/merged_GI/standardized_shared/std_mrobject_steiger_PheWASClust_PVE_240930a.rds"))
...
# trait based
#ti_instr_rmd_top5 <- trait.info_top5 %>% filter(chr_pos %in% remove_variants_names)
#ti_instr_rmd_top5_info <- merge(trait.info_top5, trait_info_unique, by = "trait", all.x = TRUE, all.y = FALSE)

# Save
phewas_table_dir <- "/pl/active/colelab/users/kjames/refinedMR/results/PheWAS_PVE/tables"
# List of top 5 hits PVE for removed variants
write.csv(instr_rmd_top5_info, paste0(phewas_table_dir, "/MR_PheWAS_PVE_RemovedVariants_top5_", exposure,"_", outcome, ".csv"))
# List of top 1 hit PVE for removed variants
write.csv(instr_rmd_top1_info, paste0(phewas_table_dir, "/MR_PheWAS_PVE_RemovedVariants_top1_", exposure,"_", outcome, ".csv"))
...
# Save tables
# All MR results
write.csv(MRres_df, paste0(phewas_table_dir, "/MR_PheWAS_PVE_Filtered_All_Results_", exposure,"_", outcome, ".csv"))
...
# Save table
write.csv(mr_ivw_df, paste0(phewas_table_dir, "/MR_PheWAS_PVE_Filtered_IVW_Results_", exposure,"_", outcome, ".csv"))
...
plots_dir <- "/pl/active/colelab/users/kjames/refinedMR/results/PheWAS_PVE/plots"

# Save
ggsave(paste0(plots_dir,"/PheWAS_PVE_MR_",exposure, "_", outcome,"_scatterplot_allmethods.png"),
       plot = MRres_p,
       device = NULL,
       path = NULL,
       scale = 1,
...
ggsave(paste0(plots_dir,"/PheWAS_PVE_MR_",exposure, "_", outcome,"_scatterplot_IVW_weightedmedn.png"),
