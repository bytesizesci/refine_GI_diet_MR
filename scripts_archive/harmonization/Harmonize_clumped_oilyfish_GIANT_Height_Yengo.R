# Goal: Harmonize of exposure and outcome data - updated to get proxy exposures
# Exposure: oily fish - GRCh37
# Outcome: height
# Author: KJS
...
# Load Exposure
ex <- readRDS("interim_data/exposure/formatted_oilyfish_clumped_noLD_nobiallelic.rds")

# Load full exposure data for when we need proxies
ex_full <- readRDS("interim_data/exposure/formatted_oilyfish_clumped_full_sumstats.rds")

# Load Outcome
# with UKB
out <- readRDS("interim_data/outcome/formatted_GCST006901_Height_Yengo_2018.rds") 
head(out) # Doesn't have a SNP rsID column
...
# Flag is an indicator we need to harmonize
...
dim(gi_needproxy) #62, brutal - SHARE!

##############
# In this data (oily fish and height) there is a variant.x and variant.y column
...
save(list= c("proxy_res", "noLD_snps_res", "missing_snps_res", "no_ex_out_snps_res"),
     file = "interim_data/merged_GI/proxydata_exp_out_proxies_oilyfish_GCST006901_Height_Yengo_2018.Rdata") 

# ~~~~~~~~~~~~~~~
# Merge with exposure-outcome data
# ~~~~~~~~~~~~~~~
# For OILYFISH and CVD there were no proxies - SKIP to SAVING
...
saveRDS(#gi_harmon,
        gi3,
        "interim_data/merged_GI/mrdat_exp_out_proxies_oilyfish_GCST006901_Height_Yengo_2018.rds")
write.csv(#gi_harmon,
          gi3, 
          "interim_data/merged_GI/mrdat_exp_out_proxies_oilyfish_GCST006901_Height_Yengo_2018.csv")

# Out of an abundance of curiosity
MRobj_std <- MendelianRandomization:: mr_input(bx = gi3$beta.exposure,
                                               bxse = gi3$se.exposure,
                                               by = gi3$beta.outcome,
                                               byse = gi3$se.outcome)
