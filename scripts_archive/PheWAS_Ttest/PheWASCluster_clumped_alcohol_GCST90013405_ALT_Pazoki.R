...
# For plots
EXP_name <- "Alcohol"
OUT_name <- "Alanine Aminotransferase"
...
key_term <- "Alcohol consumed" #"Oily fish intake", note- alcohol consumed isn't the direct metric but should be a good proxy
...
df <- readRDS(paste0("interim_data/merged_GI/standardized_shared/std_mrdat_", exposure, "_", outcome ,".rds")) #_GRCh37
...
# FIX SPECIFIC SLOT
mrobj <- readRDS("interim_data/merged_GI/standardized_shared/std_mrobject_steiger_PheWASClust_240925e.rds")
...
# Save
#saveRDS(mrobj, "interim_data/merged_GI/standardized_shared/std_mrobject_steiger_PheWASClust_240925c.rds")
saveRDS(mrobj, "interim_data/merged_GI/standardized_shared/std_mrobject_steiger_PheWASClust_241011.rds")
...
#~~~~~~~~~~~~~~~~~~~~~
# 12b. Prioritize list for poster presentation
#~~~~~~~~~~~~~~~~~~~~~
sig_ind_readable <- read.csv("/pl/active/colelab/users/kjames/enviroMR/results/julie_collab/PheWASCluster/tables/MR_PheWASCluster_SusSNPs_RelatedTraits_alcohol_GCST90013405_ALT_Pazoki.csv")
unique(sig_ind_readable$variant)
# For the poster, (taking a big story and making it smaller) create a table of the first occurrence per SNP
filtered_data <- sig_ind_readable %>%
  distinct(variant, .keep_all = TRUE)
# Save
write.csv(filtered_data, paste0(phewas_table_dir, "/MR_PheWASCluster_SusSNPs_RelatedTraits_FIRSTOCCURRENCE_SMALLLIST_FORPOSTER", exposure,"_", outcome, ".csv"))
