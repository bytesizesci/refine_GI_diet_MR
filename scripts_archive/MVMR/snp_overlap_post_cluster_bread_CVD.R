...
#
# This builds from the "gcor_kmeans_<trait>_<outcome>_v2.R" script, which builds clusters.
#
# Author: KJS
...
clst <- read.csv("interim_data/MVMR/whiteXwholegrain_CVD/plots/whiteXwholegrain_CVD_hclust_complete_cut_3.csv")
clst$trait_id <- gsub("ldsc", "gwas", clst$trait_id)
clst$trait_id <- gsub(".bgz", "", clst$trait_id)

# Load list of SNP-trait connections
sus <- read.csv("results/PheWAS_Cluster/tables/MR_PheWASCluster_SusSNPs_RelatedTraits_FilePaths_whiteXwholegrain_GCST90132314_CVD_Aragam.csv")
...
# Save dat3
write.csv(top_traits4, paste0("interim_data/MVMR/bread_LDLc/",EXP_pheno,"_",OUT_pheno,"_hclust_maxh2obs.csv"))
