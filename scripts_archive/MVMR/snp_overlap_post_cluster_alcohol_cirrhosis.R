...
clst <- read.csv("interim_data/MVMR/alcohol_cirrhosis/plots/alcohol_cirrhosis_hclust_complete_cut_5.csv")
clst$trait_id <- gsub("ldsc", "gwas", clst$trait_id)
clst$trait_id <- gsub(".bgz", "", clst$trait_id)

# Load list of SNP-trait connections
sus <- read.csv("results/PheWAS_Cluster/tables/MR_PheWASCluster_SusSNPs_RelatedTraits_FilePaths_alcohol_GCST90319877_cir_Ghouse.csv")
...
# Save dat3
write.csv(top_traits4, paste0("interim_data/MVMR/oilyfish_tg/",EXP_pheno,"_",OUT_pheno,"_hclust_maxh2obs.csv"))
