...
# Save
write.table(conf_snp, paste0("interim_data/MVMR/whiteXwholegrain_LDL/exp_clumped_snps_",diet_trait,"_MVMR_GIclump.txt"), row.names = FALSE, quote = FALSE)
...
# Save
write.table(mvmr_snps, paste0("interim_data/MVMR/whiteXwholegrain_LDL/all_clumped_snps_",diet_trait,"_MVMR_GIclump.txt"), row.names = FALSE, quote = FALSE)
...
# Save
write.table(unq_conf_snp, paste0("interim_data/MVMR/whiteXwholegrain_LDL/unique_snps_",diet_trait,"_MVMR_GIclump.txt"), row.names = FALSE, quote = FALSE) #doesn't have diet
...
# Go run MVMR_plink_clump_bread_LDL_clumpGI.sh
...
length(unique(clumped_data$SNP)) # all unique, 1954
# This is what we will use for MVMR!
# Now we need to re-extract all the final clumped SNPs from the exposures

# Save
write.table(clumped_data, paste0("interim_data/MVMR/whiteXwholegrain_LDL/clumped_",diet_trait,"_MVMR_GIsnps.txt"), row.names = FALSE, quote = FALSE)

# Load if needed
clumped_data <- read.table(paste0("interim_data/MVMR/whiteXwholegrain_LDL/clumped_",diet_trait,"_MVMR_GIsnps.txt"), header = TRUE)
...
# Now add in LDL-C (outcome)
trait <- "LDL"
out <- readRDS("interim_data/outcome/formatted_GCST90239658_LDL_Graham.rds") 
# Extract SNPs from GWAS
...
df_out_subset2 <- out[out$chr_pos %in% snp_list_noA0A1,] #2295, why is this greater than the snp_list?

# Investigating x2
length(unique(df_out_subset2$chr_pos)) #1949
nrow(unique(df_out_subset2)) #2295
dim(na.omit(df_out_subset2)) #1974, there are a lot of NAs in the duplicate SNP rows
...
length(duplicated_positions) #25, expected 1974-1974=25
...
res = dup_df[dup_df$variant %in% snp_list,] #18 out of 25, pretty good

# Merge res with no_dups_df
out_mr_snps <- rbind(no_dups_df, res)

# Save subsetted SNPs in case R runs out of memory and closes
saveRDS(df_out_subset2, paste0("interim_data/MVMR/",diet_trait,"/df_",trait,"_subset.rds"))
saveRDS(out_mr_snps, paste0("interim_data/MVMR/",diet_trait,"/df_",trait,"_subset_1941.rds"))
...
df_subset4 <- df_subset4 %>% rename(variant = variant.50_irnt)
...
df_subset7 <- df_subset7 %>% rename(variant = variant.LDL)
...
# Complete only
mvmr_dat <- na.omit(mvmr_dat) #1908
...
# Save
write.csv(sres, paste0("interim_data/MVMR/whiteXwholegrain_LDL/",diet_trait,"_MVMR_F_GIstrength.txt"))
 
# Pleiotropy
pres <- pleiotropy_mvmr(r_input = F.data, gencov = gencov) 
pres
# Save
write.csv(pres, paste0("interim_data/MVMR/whiteXwholegrain_LDL/",diet_trait,"_MVMR_Q_GIheterogeneity.txt"))
...
# Save
write.csv(res, paste0("interim_data/MVMR/whiteXwholegrain_LDL/",diet_trait,"_MVMR_IVW.txt"))
...
tmp <- mvmr_dat %>% select(variant.x, phewas_alt, effect_allele.exposure.whiteXwholegrain, beta.exposure.whiteXwholegrain, effect_allele.outcome.LDL, beta.outcome.LDL)

# Harmonize alcohol to the other exposures
mvmr_dat_harm <- harmonize_alleles(
  gi = mvmr_dat,
  effect_allele_exposure = "phewas_alt",
  effect_allele_outcome = "effect_allele.exposure.whiteXwholegrain",
  other_allele_exposure = "phewas_ref",
  other_allele_outcome = "other_allele.exposure.whiteXwholegrain",
  beta_outcome = "beta.exposure.whiteXwholegrain",
  eaf_outcome = "eaf.exposure.whiteXwholegrain",
  oaf_outcome = "oaf.exposure.whiteXwholegrain",
  chr_outcome = "chr.exposure.whiteXwholegrain",
  pos_outcome = "pos.exposure.whiteXwholegrain"
)

tmp1 <- mvmr_dat_harm %>% select(variant.x, phewas_alt, effect_allele.exposure.whiteXwholegrain, beta.exposure.whiteXwholegrain, effect_allele.outcome.LDL, beta.outcome.LDL)

# Now try outcome and an exposure
mvmr_dat_harm2 <- harmonize_alleles(
  gi = mvmr_dat_harm,
  effect_allele_exposure = "phewas_alt",
  effect_allele_outcome = "effect_allele.outcome.LDL",
  other_allele_exposure = "phewas_ref",
  other_allele_outcome = "other_allele.outcome.LDL",
  beta_outcome = "beta.outcome.LDL",
  eaf_outcome = "eaf.outcome.LDL",
  oaf_outcome = "oaf.outcome.LDL",
  chr_outcome = "chr.outcome.LDL",
  pos_outcome = "pos.outcome.LDL"
)

tmp2 <- mvmr_dat_harm2 %>% select(variant.x, phewas_alt, effect_allele.exposure.whiteXwholegrain, beta.exposure.whiteXwholegrain, effect_allele.outcome.LDL, beta.outcome.LDL)
...
diet <- read.table("/pl/active/colelab/users/kjames/refinedMR/interim_data/ldsc/processed_results/diet_gcor_results.txt")
...
colnames(diet) <- gcor_header

# ~~~~~~~~~~~~~~~~~~~~~~~~~
# Run this chunk on diet
# ~~~~~~~~~~~~~~~~~~~~~~~~~
# Identified in 04_snp_overlap.R
exposure_traits2 <- c("23098_irnt",
                      "50_irnt",
                      "23106_irnt",
                      "1448_3") # bread type whole grain
...
diet_full <- diet %>%
  ...
  bind_rows(diet)  # combine original and flipped

# Now cast
diet_wide_matrix <- reshape2::dcast(diet_full, p1 ~ p2, value.var = "rg")

# Set rownames
rownames(diet_wide_matrix) <- diet_wide_matrix[,"p1"]
diet_wide_matrix <- diet_wide_matrix %>% select(-p1)
str(diet_wide_matrix)

# Add diagonal
diag(diet_wide_matrix) <- 1

# Add ".ldsc.imputed_v3.both_sexes.tsv.bgz" to exposure_traits
phewas_traits_long <- paste0(exposure_traits2, ".ldsc.imputed_v3.both_sexes.tsv.bgz")

# Now only keep rows and columns that are in the trait list
# Filter columns
diet_wide_matrix_sub <- diet_wide_matrix %>%
  select(all_of(phewas_traits_long)) 

# Filter rows; have to turn the df into a tibble
diet_wide_matrix_sub <- diet_wide_matrix_sub %>%
  tibble::rownames_to_column(var = "trait")
diet_wide_matrix_sub <- filter(diet_wide_matrix_sub, trait %in% phewas_traits_long)

# Sort
desired_row_order <- colnames(diet_wide_matrix_sub)[-1]
diet_wide_matrix_sub <- diet_wide_matrix_sub %>%
...
# Check
str(diet_wide_matrix_sub)

# ~~~~~~~~~~ Run MVMR with genetic correlation data
# Run MVMR
library(MVMR)
F.data <- format_mvmr(BXGs = cbind(mvmr_dat_harm2$beta.23098_irnt,
                                   mvmr_dat_harm2$beta.50_irnt,
                                   mvmr_dat_harm2$beta.23106_irnt,
                                   mvmr_dat_harm2$beta.exposure.whiteXwholegrain), #beta of exposures
                      BYG = mvmr_dat_harm2$beta.outcome.LDL, # beta of outcome
                      seBXGs = cbind(mvmr_dat_harm2$se.23098_irnt,
                                     mvmr_dat_harm2$se.50_irnt,
                                     mvmr_dat_harm2$se.23106_irnt,
                                     mvmr_dat_harm2$se.exposure.whiteXwholegrain), # se of exposures
                      seBYG = mvmr_dat_harm2$se.outcome.LDL, # se of outcome
...
seBXGs <- matrix(cbind(mvmr_dat_harm2$se.23098_irnt,
                       mvmr_dat_harm2$se.50_irnt,
                       mvmr_dat_harm2$se.23106_irnt,
                       mvmr_dat_harm2$se.exposure.whiteXwholegrain),
                 byrow = TRUE,
                 ncol=4)
gencov <- phenocov_mvmr(diet_wide_matrix_sub, seBXGs)
str(gencov)
