...
# THIS WILL DIFFER BASED ON THE EXPOSURE-OUTCOME PAIR
# (Code is automatic, though)
# 
# Add the missing column
#if (!(missing_columns %in% colnames(wide_df))) {
#  wide_df[[missing_columns]] <- NA #name sets to the value stored in "missing_columns"
#}

# Add the missing row "78..."
#if (!(missing_rows %in% rownames(wide_df))) {
#  new_row <- data.frame(matrix(NA, nrow = 1, ncol = ncol(wide_df)))
#  colnames(new_row) <- colnames(wide_df)
#  rownames(new_row) <- missing_rows
...
label_map <- df %>% select(p1, p1_description) %>% distinct()
label_map <- setNames(label_map$p1_description, label_map$p1)
...
# Step 4: Reorder matrix based on clustering
wide_ordered <- wide_matrix[ordered_traits, ordered_traits]

# Step 5: Keep upper triangle and convert to long format
