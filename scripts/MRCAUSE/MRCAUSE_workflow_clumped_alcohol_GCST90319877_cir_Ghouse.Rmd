---
title: "MRCAUSE_workflow_clumped_alcohol_GCST90319877_cir_Ghouse"
author: "KJS"
date: "2024-09-16"
...
# Load outcome data
X2 <- readRDS("/pl/active/colelab/users/kjames/enviroMR/interim_data/outcome/formatted_GCST90319877_cir_Ghouse.rds")
head(X2)
str(X2)
```
...
#Some of the outcome data sets (CVD) didn't have a rsID/SNP column. Thus, we have to merge using variant ID. Create a variant ID column in X1 (diet data) that matches the X2 format (chr:bp:other:effect)
# Cirrhosis is okay, comment out next few lines
...
X <- cause::gwas_merge(X1, X2, snp_name_cols = c("SNP", "SNP"), 
...
# Prune X
Xp <- X[X$snp %in% variants_pruned$V2,] #3775
