...
# Load exposure data
X1 <- read.table(gzfile("/pl/active/colelab/common/published_gwas/diet/cole_naturecomm_2020/BOLTlmm_UKB_genoQCEURN455146_v3_diet_oilyfish_overallfreq.1329.average_QT_BgenSnps_mac20_maf0.005_info0.6.gz"), header = TRUE)
head(X1)
...
# Also annoying, in this outcome dataset the alleles are lowercase
...
Run this for outcomes missing SNP/rsID
...
# Step 4: Fit CAUSE
res_5e8 <- cause(X=X, variants = Xp$snp, param_ests = params, pval_thresh = 0.00000005) # got a warning about p-values so trying this thresh
res <- cause(X=X, variants = Xp$snp, param_ests = params, force=TRUE) # ignore p-values

# Save - results are going into their specific exposure-outcome folder
save(list=c("res", "res_5e8"), file = paste0("/pl/active/colelab/users/kjames/enviroMR/results/julie_collab/MRCAUSE/", exposure, "_", outcome,"/MRCAUSE_res_", exposure, "_", outcome, ".Rdata"))
