# Goal: Harmonize of exposure and outcome data - full exposure data for proxies
# Exposure: whiteXwhole grain
# Outcome: LDL
# Author: KJS
...
# For bread/LDL there are repeat columns we can simplify
identical(gi3$snp.x, gi3$snp.y)
gi3 <- gi3 %>% rename(SNP = SNP.x,
                      SNP_proxy = SNP) %>% select(-SNP.y)
...
# Save
saveRDS(gi3, "interim_data/merged_GI/mrdat_exp_out_proxies_whiteXwholegrain_GCST90239658_LDL_Graham.rds")
write.csv(gi3, "interim_data/merged_GI/mrdat_exp_out_proxies_whiteXwholegrain_GCST90239658_LDL_Graham.csv")
