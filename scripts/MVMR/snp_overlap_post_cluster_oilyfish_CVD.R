...
clst <- read.csv("interim_data/MVMR/oilyfish_CVD/plots/oilyfish_CVD_hclust_complete_cut_3.csv")
clst$trait_id <- gsub("ldsc", "gwas", clst$trait_id)
clst$trait_id <- gsub(".bgz", "", clst$trait_id)

# Load list of SNP-trait connections
sus <- read.csv("results/PheWAS_Cluster/tables/MR_PheWASCluster_SusSNPs_RelatedTraits_FilePaths_oilyfish_GCST90132314_CVD_Aragam.csv")
