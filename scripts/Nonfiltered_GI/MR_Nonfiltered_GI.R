...
for (i in seq_along(mrobj)){
  ...
  for (j in 1:max(n_rows_all, n_rows_stgfilt, n_rows_ttestfilt, n_rows_pvefilt)) {
    if (j <= n_rows_all) {
  ...
  write.csv(df_pub, paste0(combined_table_dir, "/MR_NonFiltered_and_Filtered_Results_Formatted", EXP_pheno,"_", OUT_pheno, ".csv"))
  ...
  write.csv(mr_wm_df, paste0(general_table_dir, "/MR_NonFiltered_and_Filtered_WeightedMedian_Results_", EXP_pheno,"_", OUT_pheno, ".csv"))
}

# Process list into all-trait table
mr_wm_full_table <- do.call("rbind", mr_wm_list)
general_table_dir <- "/pl/active/colelab/users/kjames/refinedMR/results/general/tables"
write.csv(mr_wm_full_table, paste0(general_table_dir, "/MR_NonFiltered_and_Filtered_WeightedMedian_AllTraitPairs_ProcessedManuscriptReady.csv"))
...
for (i in seq_along(mrobj)){
  ...
  #~~~~~~~~~~~~~
  # "Core" MR: Run IVW with random effects due to large amounts of heterogeneity
  #~~~~~~~~~~~~~
  mrdat_corefilt <- mrdat[(mrdat$steiger_dir == "TRUE" | mrdat$steiger_pval >= 0.05) &
                            (mrdat$PheWAS_ttest_keep == "TRUE") &
                            (mrdat$PheWAS_PVE_keep == "TRUE"),]
  ...
  # Save the combined table
  general_table_dir <- "/pl/active/colelab/users/kjames/refinedMR/results/general/tables"
  write.csv(mr_ivw_randomeffects_df, paste0(general_table_dir, "/MR_NonFiltered_and_Filtered_IVW_RandomEffects_Results_", EXP_pheno,"_", OUT_pheno, ".csv"))
...
# Save whole table
write.csv(ivw_re_df, paste0(general_table_dir, "/MR_NonFiltered_and_Filtered_IVW_RandomEffects_Results_AllTraitPairs.csv"))
...
write.csv(ivw_re_df_processed_2, paste0(general_table_dir, "/MR_NonFiltered_and_Filtered_IVW_RandomEffects_Results_AllTraitPairs_ProcessedManuscriptReady.csv"))
