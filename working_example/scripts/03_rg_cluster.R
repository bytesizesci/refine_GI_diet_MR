...
key <- read.csv("~/working_example/data/UKBB GWAS Imputed v3 - File Manifest Release 20180731 - Description Lookup.csv", header = FALSE)
...
# Ensure all pairs are in the upper triangle (P1 < P2)
# Maintain the relationship of all 3 types of id columns
dfmu2 <- dfmu %>%
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Step 2: Apply gcor data to exposure and outcome of interest
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# Have to re-check that everything will be in the upper triangle.
# Maintains the relationship of all 3 types of id columns
df <- df %>%
  mutate(swap = p1_description > p2_description) %>%
  mutate(
    p1_final = if_else(swap, p2, p1),
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Step 4: Hierarchical clustering prep
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Plot based on rg value instead of alphabetical order
hc_temp = hclust(as.dist(1 - wide_matrix), method = "complete") # hclust(dist(wide_matrix)) - dist() is for calculating distances but I just need to convert my correlations (similarity) into distances
ordered_traits = hc_temp$labels[hc_temp$order]

# How many clusters? Look for major height changes
plot(rev(hc_temp$height), type = "b") #reverse (rev()) of height
...
# Build label map from p1 and p2! (gets all traits even if they are only represented in p2)
label_map <- df %>%
...
ggsave(paste0(plot_dir, "/", EXP_pheno, "_", OUT_pheno, "_hc_complete_heatmap.png"))

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Step 5: Hierarchical clustering
# USER INPUT REQUIRED TO DEFINE k
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# Balancing biological interpretation of groups & height difference between branches
k=5 # USER INPUT REQUIRED - define how many clusters
...
write.csv(cluster_df_filename, paste0(table_dir, "/", EXP_pheno, "_", OUT_pheno, "_hclust_complete_", k,"clusters.csv"), row.names = FALSE)

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Step 6: Hierarchical clustering plotting
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# For readability, add labels - helps with plotting
...
# Save
png(paste0(res_dir,"/",EXP_pheno,"_", OUT_pheno, "_hc_complete_heatmap_color.png"),
    width = 5000,
    height = 5000,
    res = 300)
...
# Gets cut off so try another plot
dend <- as.dendrogram(hc)  # or your dend object
dend <- dendextend::set(dend, "labels_cex", 0.5)
dend <- dendextend::set(dend, "labels_colors", "black")  # optional

# Rotate the plot (labels appear on top, easier to read)
png(paste0(res_dir,"/",EXP_pheno,"_", OUT_pheno, "_hc_complete_heatmap_color.png"),
    width = 5000,
    height = 5000,
    res = 300)
